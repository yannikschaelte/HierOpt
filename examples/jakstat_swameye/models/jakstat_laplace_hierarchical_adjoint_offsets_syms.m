function [model] = jakstat_laplace_hierarchical_adjoint_offsets_syms()
...
model.param = 'log10';
model.sym.k = [Omega_cyt,Omega_nuc,offset_pSTAT,offset_tSTAT,scale_pSTAT,scale_tSTAT];
...
model.sym.y(1) = offset_pSTAT + scale_pSTAT*(1/init_STAT*(pSTAT + 2*pSTAT_pSTAT));
model.sym.y(2) = offset_tSTAT + scale_tSTAT*(1/init_STAT*(STAT + pSTAT + 2*(pSTAT_pSTAT)));
model.sym.y(3) = u(1);
...
for j = 1:length(model.sym.y)
	model.sym.Jy(j) = sym(['log(2*sigma_y_' num2str(j-1) ') + abs((y_' num2str(j-1) '-my_' num2str(j-1) ')/(sigma_y_' num2str(j-1) '))']);
end
