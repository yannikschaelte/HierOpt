function [model] = jakstat_laplace_hierarchical_adjoint_syms()
...
model.param = 'log10';
model.sym.k = [Omega_cyt,Omega_nuc,scale_pSTAT,scale_tSTAT];
...
model.sym.y(1) = scale_pSTAT*(offset_pSTAT + 1/init_STAT*(pSTAT + 2*pSTAT_pSTAT));
model.sym.y(2) = scale_tSTAT*(offset_tSTAT + 1/init_STAT*(STAT + pSTAT + 2*(pSTAT_pSTAT)));
model.sym.y(3) = u(1);
