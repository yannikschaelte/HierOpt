function varargout = logLikelihood_Zheng(xi,D,simfct,options)
% loglikelihood function incorporating additional scaling factors for each
% time point
...
switch options.llh.approach
    ...
    case 'standard'
        c = 10.^xi(end-4:end-1)';
        sigma2 = 10.^xi(end)*ones(1,1); %only one sigma so far
        
        sol.sy(:,:,end+1:end+5) = 0;
        if nderiv > 0
            dc = zeros(1,4,numel(xi));
            dc(1,:,end-4:end-1,:) = diag(c)*log(10);
            dsigma2 = zeros(1,4,numel(xi));
            dsigma2(1,:,end) = log(10)*sigma2;
        end
        y_ch = bsxfun(@minus,D.my,bsxfun(@times,permute(c,[1,2,4,3]),sol.y));
        if nderiv > 0
            dcy_cdy = bsxfun(@times,dc,sol.y) +...
                bsxfun(@times,permute(c,[1,2,4,3]),sol.sy);
        end
        switch options.llh.distribution
            case 'laplace'
                nlogL = sum(sum(nansum(bsxfun(@times,~isnan(D.my),log(2*sigma2))+...
                    bsxfun(@rdivide,abs(y_ch),sigma2),1),3),2);
                if nargout > 1
                    dnlogL = permute(sum(sum(nansum(...
            ...
            case 'normal'
                nlogL = 0.5*(sum(sum(nansum(bsxfun(@times,~isnan(D.my),log(2*pi*sigma2))+...
                    bsxfun(@rdivide,bsxfun(@power,y_ch,2),sigma2),1),3),2));
                if nargout > 1
                    dnlogL = 0.5*(permute(sum(sum(nansum(...
...
if options.llh.prior.flag
    nlogL = nlogL + 0.5*nansum((xi-options.llh.prior.mean).^2./...
        options.llh.prior.sigma2);
    if nargout > 1
        dnlogL = nansum([dnlogL,(xi-options.llh.prior.mean)./...
