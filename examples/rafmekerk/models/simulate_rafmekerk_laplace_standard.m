% simulate_rafmekerk_laplace_standard.m is the matlab interface to the cvodes mex
...
% USAGE:
% ======
% [...] = simulate_rafmekerk_laplace_standard(tout,theta)
% [...] = simulate_rafmekerk_laplace_standard(tout,theta,kappa,data,options)
% [status,tout,x,y,sx,sy] = simulate_rafmekerk_laplace_standard(...)
%
% INPUTS:
% =======
...
function varargout = simulate_rafmekerk_laplace_standard(varargin)
...
sol = ami_rafmekerk_laplace_standard(tout,theta(1:28),kappa(1:2),options_ami,plist,xscale,init,data);
if(nargout>1)
