% simulate_rafmekerk_hierarchical_forward.m is the matlab interface to the cvodes mex
...
% USAGE:
% ======
% [...] = simulate_rafmekerk_hierarchical_forward(tout,theta)
% [...] = simulate_rafmekerk_hierarchical_forward(tout,theta,kappa,data,options)
% [status,tout,x,y,sx,sy] = simulate_rafmekerk_hierarchical_forward(...)
%
% INPUTS:
% =======
...
function varargout = simulate_rafmekerk_hierarchical_forward(varargin)
...
sol = ami_rafmekerk_hierarchical_forward(tout,theta(1:12),kappa(1:2,:),options_ami,plist,pbar(plist+1),xscale,init,data);
if(nargout>1)
