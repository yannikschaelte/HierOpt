% simulate_RafMekErk.m is the matlab interface to the cvodes mex
...
% USAGE:
% ======
% [...] = simulate_RafMekErk(tout,theta)
% [...] = simulate_RafMekErk(tout,theta,kappa,data,options)
% [status,tout,x,y,sx,sy] = simulate_RafMekErk(...)
%
% INPUTS:
% =======
...
function varargout = simulate_RafMekErk(varargin)
...
if(max(options_ami.sens_ind)>20)
...
sol = ami_RafMekErk(tout,theta(1:20),kappa(1:2),options_ami,plist,pbar(plist+1),xscale,init,data);
if(nargout>1)
