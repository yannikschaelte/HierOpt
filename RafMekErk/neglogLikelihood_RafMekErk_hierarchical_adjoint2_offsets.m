function [varargout] = neglogLikelihood_RafMekErk_hierarchical_adjoint2(varargin)
...
for j = 1:n_e
    ...
    tout_j = D(j).t;
    c = ones(1,8);
    kappa_j = [ data.conditions(j,:) c ]'; % default values for scalings
    data_j = []; % no data
    sol = simulate_RafMekErk_hierarchical_adjoint2(tout_j,theta,kappa_j,data_j,amioptions);
    if sol.status < 0
...
% scalings
for j=1:8
    options.obs(j).variance = 'multiple';
    options.obs(j).proportionality = 'multiple';
    options.obs_groups.proportionality{j} = j;
    options.obs_groups.variance{j} = j;
end

[c,sigma2,c_by_y,sigma2_by_y] = opt_scalings_normal(sim,D,options);
...
if nargout > 1
    ...
    for ie = 1:n_e
        for ir = 1:n_r
            ...
            kappa_re = [ data.conditions(ie,:) c_re ]'; % default values for scalings
            ...
            sol = simulate_RafMekErk_hierarchical_adjoint2(tout_e,theta,kappa_re,data_re,amioptions);
            if sol.status < 0
