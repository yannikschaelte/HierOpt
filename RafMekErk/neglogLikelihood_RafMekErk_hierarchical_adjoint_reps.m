...
repmode = 'multiple';
%y(1)
options.obs(1).variance = repmode;
options.obs(1).proportionality = repmode;
%y(2)
options.obs(2).variance = repmode;
options.obs(2).proportionality = repmode;
...
if nargout > 1
    ...
    for ie = 1:n_e
%                 for ir = 1:n_r
%         values for c,sigma2,b should be the same for all replicates
        clear data_e
        ...
        for ir = 1:n_r
           c_re = c_by_y(:,:,ir,ie);
           c_re = reshape(c_re,1,[]);
           kappa_re = [ kappa(ie,:) c_re ]';
           kappa_e(:,ir) = kappa_re;
        end
        
        % compute sigma
        sigma2_e = zeros(n_t,n_y,n_r);
        for ir = 1:n_r
            sigma2_re = sigma2_by_y(:,:,ir,ie);
            sigma2_re = reshape(sigma2_re,1,[]);
            sigma2_re = repmat(sigma2_re,n_t,1);
            sigma2_e(:,:,ir) = sigma2_re;
        end
        
        my_e = D(ie).my(:,:,:); % data
        data_e.t = tout_e;
        data_e.Y = my_e;
        data_e.Sigma_Y = sqrt(sigma2_e);
        data_e.condition = kappa_e;
        data_e = amidata(data_e);
        
        sol = simulate_RafMekErk_hierarchical_adjoint_reps(tout_e,theta,kappa_e,data_e,amioptions);
        if sol.status < 0
