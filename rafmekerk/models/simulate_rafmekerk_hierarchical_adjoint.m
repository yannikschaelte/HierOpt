% simulate_rafmekerk_hierarchical_adjoint.m is the matlab interface to the cvodes mex
...
% USAGE:
% ======
% [...] = simulate_rafmekerk_hierarchical_adjoint(tout,theta)
% [...] = simulate_rafmekerk_hierarchical_adjoint(tout,theta,kappa,data,options)
% [status,tout,x,y,sx,sy] = simulate_rafmekerk_hierarchical_adjoint(...)
%
% INPUTS:
% =======
...
function varargout = simulate_rafmekerk_hierarchical_adjoint(varargin)
...
sol = ami_rafmekerk_hierarchical_adjoint(tout,theta(1:12),kappa(1:4),options_ami,plist,pbar(plist+1),xscale,init,data);
if(nargout>1)
