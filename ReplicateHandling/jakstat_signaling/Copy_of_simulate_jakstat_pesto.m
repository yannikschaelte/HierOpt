...
%   this routine was generated using AMICI commit c51bd318a8468eaafbac2e8edcd12ee7240d6323 in branch feature_scalings in repo https://github.com/icb-dcm/amici.
%
% USAGE:
% ======
% [...] = simulate_jakstat_pesto(tout,theta)
% [...] = simulate_jakstat_pesto(tout,theta,kappa,data,options)
% [status,tout,x,y,sx,sy] = simulate_jakstat_pesto(...)
%
% INPUTS:
% =======
...
function varargout = simulate_jakstat_pesto(varargin)
...
if(options_ami.sensi<2)
    sol = ami_jakstat_pesto(tout,theta(1:17),kappa(1:2),options_ami,plist,pbar(plist+1),xscale,init,data);
else
    sol = ami_jakstat_pesto_o2(tout,theta(1:17),kappa(1:2),options_ami,plist,pbar(plist+1),xscale,init,data);
end
if(options_ami.sensi == 2)
