% simulate_RafMekErk_hierarchical_adjoint_reps.m is the matlab interface to the cvodes mex
...
% USAGE:
% ======
% [...] = simulate_RafMekErk_hierarchical_adjoint_reps(tout,theta)
% [...] = simulate_RafMekErk_hierarchical_adjoint_reps(tout,theta,kappa,data,options)
% [status,tout,x,y,sx,sy] = simulate_RafMekErk_hierarchical_adjoint_reps(...)
%
% INPUTS:
% =======
...
function varargout = simulate_RafMekErk_hierarchical_adjoint_reps(varargin)
...
if(options_ami.sensi<2)
    sol = ami_RafMekErk_hierarchical_adjoint_reps(tout,theta(1:12),kappa(1:4),options_ami,plist,pbar(plist+1),xscale,init,data);
else
    sol = ami_RafMekErk_hierarchical_adjoint_reps_o2(tout,theta(1:12),kappa(1:4),options_ami,plist,pbar(plist+1),xscale,init,data);
end
if(options_ami.sensi == 2)
    if(~(options_ami.sensi_meth==2))
        ...
        sol.s2x = reshape(sol.sx(:,4:end,:),length(tout),3,12,length(options_ami.sens_ind));
        sol.s2y = reshape(sol.sy(:,3:end,:),length(tout),2,12,length(options_ami.sens_ind));
        sol.s2sigmay = reshape(sol.ssigmay(:,3:end,:),length(tout),2,12,length(options_ami.sens_ind));
        s2z = zeros(size(sol.z,1),0,12,length(options_ami.sens_ind));
        s2sigmaz = zeros(size(sol.z,1),0,12,length(options_ami.sens_ind));
        s2rz = zeros(size(sol.z,1),0,12,length(options_ami.sens_ind));
        for iz = 1:0
            sol.s2z(:,iz,:,:) = reshape(sol.sz(:,((iz-1)*(12+1)+2):((iz-1)*(12+1)+12+1),:),options_ami.nmaxevent,1,12,length(options_ami.sens_ind));
            sol.s2sigmaz(:,iz,:,:) = reshape(sol.ssigmaz(:,((iz-1)*(12+1)+2):((iz-1)*(12+1)+12+1),:),options_ami.nmaxevent,1,12,length(options_ami.sens_ind));
            sol.s2rz(:,iz,:,:) = reshape(sol.srz(:,((iz-1)*(12+1)+2):((iz-1)*(12+1)+12+1),:),options_ami.nmaxevent,1,12,length(options_ami.sens_ind));
        end
        sol.sx = sol.sx(:,1:3,:);
        sol.sy = sol.sy(:,1:2,:);
        sol.ssigmay = sol.ssigmay(:,1:2,:);
        if(iz>0)
            sol.sz = sz;
            sol.ssigmaz = ssigmaz;
            sol.srz = srz;
